"use strict";
var promises = require("node:fs/promises"), node_path = require("node:path"), yaml = require("yaml");
class GitHubFileReader {
  baseUrl;
  headers;
  constructor(baseUrl, headers = {}) {
    this.baseUrl = baseUrl.replace(/\/+$/, ""), this.headers = headers;
  }
  ...
  async readDir(dirPath) {
    try {
      const url = new URL(this.baseUrl), [, owner, repo, branch = "", ...rest] = url.pathname.split("/"), dirSlug = node_path.join(`repos/${owner}/${repo}/contents`, ...rest, dirPath), apiUrl = new URL(dirSlug, "https://api.github.com");
...
class LocalFileReader {
  ...
  async readFile(filePath) {
    try {
      const fullPath = node_path.join(this.basePath, filePath);
      return {
        exists: !0,
        content: await promises.readFile(fullPath, "utf-8")
      };
    } catch {
      return {
        exists: !1,
  ...
  async readDir(dirPath) {
    try {
      const fullPath = node_path.join(this.basePath, dirPath);
      return (await promises.readdir(fullPath, { withFileTypes: !0 })).filter((entry) => entry.isDirectory()).map((entry) => entry.name);
    } catch {
      return [];
    }
  }
...
async function getMonoRepo(fileReader) {
  const expandWildcards = async (patterns) => Promise.all(
    patterns.map(async (pattern) => {
      ...
      const [baseDirRaw = ""] = pattern.split("/*"), baseDir = baseDirRaw.replace(/\/$/, "");
      return (await fileReader.readDir(baseDir).catch(() => [])).map((dir) => node_path.join(baseDir, dir));
    })
...
async function validatePackage(fileReader, packagePath) {
  const packageName = packagePath || ROOT_PACKAGE_NAME, errors = [], requiredFiles = [
    ...
    requiredFiles.map(async (file) => {
      const filePath = packagePath ? node_path.join(packagePath, file) : file, result = await fileReader.readFile(filePath);
      return { file, ...result };
    })
...
async function validateTemplate(fileReader, packages = [""]) {
  ...
  const missingEnvTemplates = packages.filter((_, i) => validations[i] && validations[i].hasSanityDep && !validations[i].hasEnvFile).map((p) => p || ROOT_PACKAGE_NAME), envExamples = ENV_TEMPLATE_FILES.join(", "), missingTemplatesStr = missingEnvTemplates.join(", ");

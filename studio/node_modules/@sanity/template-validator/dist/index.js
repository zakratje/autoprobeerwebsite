...
class GitHubFileReader {
  ...
  async readDir(dirPath) {
    try {
      const url = new URL(this.baseUrl), [, owner, repo, branch = "", ...rest] = url.pathname.split("/"), dirSlug = join(`repos/${owner}/${repo}/contents`, ...rest, dirPath), apiUrl = new URL(dirSlug, "https://api.github.com");
...
class LocalFileReader {
  ...
  async readDir(dirPath) {
    try {
      ...
      return (await readdir(fullPath, { withFileTypes: !0 })).filter((entry) => entry.isDirectory()).map((entry) => entry.name);
    } catch {
      return [];
    }
  }
